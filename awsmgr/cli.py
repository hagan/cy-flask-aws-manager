...
from awsmgr.app.blueprints.pulumi.services.dynamodb import pulumi_dynamodb_instance_func
from awsmgr.app.blueprints.pulumi.services.setup_lambda import pulumi_lambda_instance_func
...
from awsmgr.app.blueprints.boto.services.dataclass import AWSMgrConfigDataClass, AWSMgrConfigDataClassException
...
DEFAULT_EC2_RESOURCE_NAME = 'my-awsmgr-ec2-server'
DEFAULT_DYNAMODB_RESOURCE_NAME = 'my-awsmgr-dynamo-db'
DEFAULT_LAMBDA_RESOURCE_NAME = 'my-awsmgr-lambda-func'
...
def create_s3(env: dict, skip_memcached: bool, bucket_name: str):
    ...
    with app.app_context():
        ...
        pulumi_setup_stack_call(
            acdc,
            lambda: pulumi_s3_bucket_func(acdc, bucket_name, printf=click.echo),
            project_name,
            stack_name,
            pulumi_project_dir,
            pulumi_home_dir,
...
def destroy_s3(env: dict, skip_memcached: bool, bucket_name: str):
    ...
    """
    click.echo(f"\t[1] Deprovisioning '{bucket_name}' S3 bucket")
    click.echo("\t[2] Initialize Flask application stack")
    app = create_app()
    with app.app_context():
        click.echo("\t[3] Initialized app context")
...
@click.option('--dynamodb-name', default=DEFAULT_DYNAMODB_RESOURCE_NAME, help=f"EC2 resource name, ie {DEFAULT_DYNAMODB_RESOURCE_NAME}")
def create_dynamodb(env: dict, skip_memcached: bool, tag_name: str, dynamodb_name: str):
    click.echo(f"\t[1] Provisioning '{dynamodb_name}' DynamoDB resource")
    click.echo("\t[2] Initialize Flask application stack")
    app = create_app()
    with app.app_context():
        click.echo("\t[3] Initialized app context")
        try:
            acdc = AWSMgrConfigDataClass(environment=env, skip_memcached=skip_memcached)
        except AWSMgrConfigDataClassException as e:
            if e.status == 1:
                click.echo(f"ERROR: {e}")
                click.echo("Hint: Define matching environment variables or pass option via cli")
            else:
                raise e
        ...
        pulumi_setup_stack_call(
            acdc,
            lambda: pulumi_dynamodb_instance_func(acdc, dynamodb_name),
            project_name,
            stack_name,
            pulumi_project_dir,
            pulumi_home_dir,
...
@click.option('--tag-name', default=DEFAULT_TAG_NAME, help="DEFAULT_TAG_NAME environment variable override")
@click.option('--dynamodb-name', default=DEFAULT_DYNAMODB_RESOURCE_NAME, help=f"EC2 resource name, ie {DEFAULT_DYNAMODB_RESOURCE_NAME}")
def destroy_dynamodb(env: dict, skip_memcached: bool, tag_name: str, dynamodb_name: str):
    click.echo(f"\t[1] Deprovisioning '{dynamodb_name}' DynamoDB resource")
    click.echo("\t[2] Initialize Flask application stack")
    app = create_app()
    with app.app_context():
        click.echo("\t[3] Initialized app context")
...
@click.option('-m', '--skip-memcached', is_flag=True, show_default=True, default=False, help="Don't poll memcached for variable")
@click.option('--tag-name', default=DEFAULT_TAG_NAME, help="DEFAULT_TAG_NAME environment variable override")
@click.option('--lambda-name', default=DEFAULT_LAMBDA_RESOURCE_NAME, help=f"Lambda resource name, ie {DEFAULT_LAMBDA_RESOURCE_NAME}")
def create_lambda(env: dict, skip_memcached: bool, tag_name: str, lambda_name: str):
    click.echo(f"\t[1] Provisioning '{lambda_name}' Lambda resource")
    click.echo("\t[2] Initialize Flask application stack")
    app = create_app()
    with app.app_context():
        click.echo("\t[3] Initialized app context")
        ...
        pulumi_setup_stack_call(
            acdc,
            lambda: pulumi_lambda_instance_func(acdc, lambda_name, printf=click.echo),
            project_name,
            stack_name,
            pulumi_project_dir,
            pulumi_home_dir,
...
@click.option('-m', '--skip-memcached', is_flag=True, show_default=True, default=False, help="Don't poll memcached for variable")
@click.option('--tag-name', default=DEFAULT_TAG_NAME, help="DEFAULT_TAG_NAME environment variable override")
@click.option('--lambda-name', default=DEFAULT_LAMBDA_RESOURCE_NAME, help=f"EC2 resource name, ie {DEFAULT_LAMBDA_RESOURCE_NAME}")
def destroy_lambda(env: dict, skip_memcached: bool, tag_name: str, lambda_name: str):
    click.echo(f"\t[1] Deprovisioning '{lambda_name}' Lambda resource")
    click.echo("\t[2] Initialize Flask application stack")
    app = create_app()
    with app.app_context():
        click.echo("\t[3] Initialized app context")
...
@click.option('-m', '--skip-memcached', is_flag=True, show_default=True, default=False, help="Don't poll memcached for variable")
@click.option('--tag-name', default=DEFAULT_TAG_NAME, help="DEFAULT_TAG_NAME environment variable override")
@click.option('--ec2-resource-name', default=DEFAULT_EC2_RESOURCE_NAME, help=f"EC2 resource name, ie {DEFAULT_EC2_RESOURCE_NAME}")
